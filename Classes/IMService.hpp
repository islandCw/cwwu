...
#define YouMe_Log( ... ) printf("_______________YouMe:");printf( __VA_ARGS__ )

struct Msg{
public:
    unsigned long long msgID = 0;   //消息ID，自己发送的没有消息ID
    XString sender = UTF8TOXString2("");              //发送者
    bool  bSelf = false ;           //是否自己发送
    unsigned int createTime = 0 ;   //发送时间
    bool bTxt = false ;             //是否文本
	XString content = UTF8TOXString2("");            //如果是文本，则是文本内容，如果是语音，则是语音检测出的文本
	XString voicePath = UTF8TOXString2("");           //语音的保存地址
    int voiceLen = 0 ;              //语音长度
};

class IMService : public IYIMLoginCallback,  public IYIMMessageCallback,  public IYIMChatRoomCallback, public IYIMDownloadCallback, public IYIMContactCallback , public IYIMAudioPlayCallback
{
public:
	...
	void OnSendMessageStatus(XUINT64 requestID, YIMErrorcode errorcode, unsigned int sendTime, bool isForbidRoom, int reasonType, XUINT64 forbidEndTime)override;
    //发送语音消息回调
	void OnSendAudioMessageStatus(XUINT64 requestID, YIMErrorcode errorcode, const XString& text, const XString& audioPath, unsigned int audioTime, unsigned int sendTime,
	...
	void OnQueryRoomHistoryMessageFromServer(YIMErrorcode errorcode, const XString& roomID, int remain, std::list<std::shared_ptr<IYIMMessage> >& messageList)override;
    //语音上传后回调
